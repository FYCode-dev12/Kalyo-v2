...
type Calendar = { id: string; displayName: string; googleCalendarId: string; color: string; isBookingTarget: boolean; showOnPublic: boolean };
...
export default function SettingsPage() {
  ...
  async function updateCalendar(id: string, changes: { showOnPublic?: boolean; isBookingTarget?: boolean }) {
    const response = await fetch('/api/admin/settings', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ calendar: { id, ...changes } }),
    });
    const body = await response.json();
    if (!response.ok) return setMessage(body.error || 'Gagal memperbarui kalender.');
    setCalendars((current) => current.map((calendar) => calendar.id === id ? { ...calendar, ...changes } : calendar));
    setMessage('Pengaturan kalender berhasil diperbarui.');
  }
  ...
  return <main className="min-h-screen bg-slate-50 p-4 sm:p-8"><div className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="rounded-2xl border bg-white p-6 shadow-sm"><h2 className="text-xl font-bold">Calendar sources</h2><p className="mt-1 text-sm text-slate-500">Atur kalender yang tampil di publik dan kalender tujuan booking.</p><div className="mt-4 divide-y">{calendars.map((calendar) => <div key={calendar.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><span className="h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: calendar.color }} /><span className="font-semibold">{calendar.displayName}</span></div><code className="mt-1 block break-all text-xs text-slate-500">{calendar.googleCalendarId}</code></div><div className="flex flex-wrap gap-3 text-sm"><label className="flex items-center gap-2"><input type="checkbox" checked={calendar.showOnPublic} onChange={(event) => updateCalendar(calendar.id, { showOnPublic: event.target.checked })} /> Tampil publik</label><label className="flex items-center gap-2"><input type="checkbox" checked={calendar.isBookingTarget} onChange={(event) => updateCalendar(calendar.id, { isBookingTarget: event.target.checked })} /> Target booking</label></div></div>)}</div></section>
  </div></main>;
}
